import { AlertTriangle, ArrowLeft, CheckCircle2, FileLock2, Save } from 'lucide-react';
...
import type { PayoutStatementData, VendorPayoutWorkspaceData } from '../../types/payouts';
...
export function PayoutDraftReview() {
    ...
    const persistDraft = async (quiet = false) => {
        ...
        try {
            const id = await savePayoutDraft({
                ...
                payoutId,
                rangeMode: statement.payout.range_mode,
                sourceRangeStart: String(statement.payout.source_range_start || '') || null,
                sourceRangeEnd: String(statement.payout.source_range_end || '') || null,
                includePriorBalance: Boolean(statement.payout.include_prior_balance),
                paymentAmount: Number(paymentAmount),
                invoiceApplications,
                notes,
                belowThresholdOverrideReason: overrideReason || null,
            });
            if (!quiet) toast.success('Draft saved', 'Exact sale and adjustment snapshots were refreshed.');
            await load();
            return id;
        } catch (reason) {
    ...
    return (
        <div className="mx-auto w-full max-w-[1500px] pb-40">
            ...
            {!statement.payout.include_prior_balance ? (
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
                ...
                <aside className="h-fit border border-[var(--color-border)] bg-[var(--color-card)] xl:sticky xl:top-4">
                    ...
                    <div className="space-y-4 p-5">
                        ...
                        <Button fullWidth variant="success" leftIcon={<CheckCircle2 className="h-4 w-4" />} isLoading={isFinalizing} onClick={markPaid}>Mark paid and lock</Button>
