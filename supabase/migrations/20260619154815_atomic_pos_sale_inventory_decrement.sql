-- Make POS sale creation and inventory decrement atomic.
--
-- Previously the app called create_pos_sale_with_items(), then each POS client
-- separately updated public.items.quantity. That let sales commit while stock
-- stayed stale if the follow-up client update failed or was blocked by RLS.
...
BEGIN
  ...
  updated_items AS (
    ...
    SET quantity = item.quantity - sale_item_quantities.quantity_sold
    FROM sale_item_quantities
    WHERE item.id = sale_item_quantities.item_id
      AND item.quantity >= sale_item_quantities.quantity_sold
    RETURNING item.id
  )
  SELECT
    (SELECT COUNT(*) FROM sale_item_quantities),
