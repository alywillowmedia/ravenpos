-- Fix POS atomic sale RPC UUID generation.
--
-- The original guardrail RPC used uuid_generate_v4(), but this production
-- database does not expose that function. RavenPOS already uses
-- gen_random_uuid() elsewhere, so keep the atomic sale behavior and switch
-- fallback ID generation to gen_random_uuid().
